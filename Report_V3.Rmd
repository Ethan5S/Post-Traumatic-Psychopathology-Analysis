...
Regardless, we can observe that all models reached a test accuracy range of 0.87 to 0.89 as well as similar True Negative and True positive rates showing that all models performed similarly despite having different methods for computing their predictions. These results may suggest that these differing model fitting methods may not be key to increasing model performance. However, other possible steps previously mentioned in the limitations and alternatives such as imputing nominal values, different feature selection methods, or possibly including more predictors in the dataset may yield better results. 
