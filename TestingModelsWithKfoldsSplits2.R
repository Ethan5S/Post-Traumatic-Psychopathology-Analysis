...
for (i in 1:5) {
  ...
  train_df <- data.frame(Xtrain, ptp3_yn = Ytrain)
  test_df <- data.frame(Xtest)
  
  rf_fit <- randomForest(ptp3_yn ~ ., data = train_df)
  ...
  rf_cross_entropies_losses[i] <- CrossEntropyLoss(pred_probs, as.numeric(Ytest))
